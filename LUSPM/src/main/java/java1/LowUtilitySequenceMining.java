...
//    private int[] prefixSum; // prefixSum[i] = sum(flatUtils[0..i-1])
    ...
    public void runAlgorithm(String input, int max_utility, int maxLength, BufferedWriter writer) throws IOException {
        ...
        System.out.println("This is the low-utility non-contiguous sequence mining algorithm");
        ...
        buffer.append("Max utility: ").append(max_utility).append("\n");
        buffer.append("Max length: ").append(maxLength).append("\n");
        ...
        System.out.println("Sequence count: " + sequenceId);
...
//                                if(utility<lowerUtility){lowerUtility=utility;}
                                ...
                                // 处理解析错误的情况
                                System.out.println("Error parsing item or utility: " + pair);
                            }
                        }
...
public static List<List<Integer>> deduplicateUtilities(List<List<Integer>> utilities) {
    ...
    public void getMaxSequence(int maxLength) throws IOException {
        StringBuilder buffer = new StringBuilder();
        buffer.append("Currently running: preprocess\n");
        System.out.println("Currently running: preprocess");
...
//            processed(sequence,utilities);
        ...
        buffer.append("Max sequences: ");
        buffer.append(maxSequenceList);
        System.out.println("Max sequences");
...
//        if (!hasProcessedSequenceList.contains(sequence) && hasKnownHighUtilitySequence.contains(sequence) == false) {
        System.out.println("Found low-utility sequence: " + sequence);
        StringBuilder buffer = new StringBuilder();
        buffer.append("Found low-utility sequence: " + sequence).append("\n");
            writer.write(buffer.toString());
            writer.newLine();
...
//            candidatesCount++;
    ...
    public void processCoreAlgorithm() throws IOException {
        System.out.println("Currently running LUSM");
        ...
        for (List<Integer> sequence : maxSequenceList) {
            ...
            if(sequence.size()>0){
                ...
                if (utilityOfSequence == -1) System.out.println("This sequence is empty!");
                candidatesCount++;
...
public void printLowUtilitySequence() throws IOException {
    writer.write("Number of low-utility non-contiguous sequences: " + patternCount + "\n");
    System.out.println("Number of low-utility sequences: " + patternCount);

    if (patternCount > 0) {
        writer.write("Output low-utility sequences:\n");
        System.out.println("Output low-utility sequences:");

        int limit = 1000000000; // Limit output to max 1,000,000,000 records (adjustable)
        int count = 0;

        for (Map.Entry<List<Integer>, Integer> entry : lowUtilityPattern.entrySet()) {
            if (count++ >= limit) break;

            String line = "Sequence: " + entry.getKey() + "  Utility: " + entry.getValue() + "\n";
            writer.write(line);
            System.out.println(line.trim());
        }
    }

    writer.write("Total number of low-utility sequences: " + lowUtilityPattern.size() + "\n");
    writer.newLine();
}
    ...
    public void showStates() throws IOException {
        ...
        buffer.append("Bitmap generation time: " + (double) runtime4 / 1000 + " s").append("\n");
        buffer.append("Longest sequence extraction time: " + (double) runtime5 / 1000 + " s").append("\n");
        buffer.append("Preprocessing time: " + (double) runtime3 / 1000 + " s").append("\n");
        buffer.append("Total runtime: " + (double) runtime / 1000 + " s").append("\n");

        buffer.append("Maximum memory usage: " + MemoryLogger.getInstance().getMaxMemory() + " MB").append("\n");
        buffer.append("Total memory: " + (double) Runtime.getRuntime().totalMemory() / 1024d / 1024d + " MB").append("\n");
        buffer.append("Free memory: " + (double) Runtime.getRuntime().freeMemory() / 1024d / 1024d + " MB").append("\n");

        buffer.append("Number of candidates (improved): " + candidatesCount).append("\n");
        ...
        System.out.println("Bitmap generation time: " + (double) runtime4 / 1000 + " s");
        System.out.println("Longest sequence extraction time: " + (double) runtime5 / 1000 + " s");
        System.out.println("Preprocessing time: " + (double) runtime3 / 1000 + " s");
        System.out.println("Total runtime: " + (double) runtime / 1000 + " s");

        System.out.println("Maximum memory usage: " + MemoryLogger.getInstance().getMaxMemory() + " MB");
        System.out.println("Total memory: " + (double) Runtime.getRuntime().totalMemory() / 1024d / 1024d + " MB");
